...
int data_array_length = 10000, write_times = 0, been_written = 0, number_of_files = 1, what_to_save = 0, total_nutrient_ml = 0, write_header = 0, total_runs = 0, eq_end_time_sec = 0, eq_start_time_sec = 0,
...
{		// get the current system time sets global variables sTime and sDate in RTC_driver.c
	...
	for(int samples = 0; samples<30; samples++)			   // sample TDS and PH every half second for 30 times
	{
		...
		sample_array_TDS[samples] = readWaterTDS();;// readWaterTDS();
		sample_array_pH[samples] = readPH();; //readPH();
		HAL_Delay(10);
	}
...
/*
    int pH_smallest_loc = 0;
    int TDS_smallest_loc = 0;
    int pH_largest_loc = 0;
    int TDS_largest_loc = 0;
    int loc = 0;
    int pH_total_increasing = 0;
    int pH_total_decreasing= 0;
    int pH_total_same = 0;
    int TDS_total_increasing = 0;
    int TDS_total_decreasing = 0;
    int TDS_total_same = 0;
*/
	for(int i = 0; i<30;i++)
	{
		...
		// find the number of increasing and decreasing data points
	...
	/*float TDS_difference = 0;
	float pH_difference = 0;
	for(int j = 1; j < 25; j++)
	{
			TDS_difference = sample_array_TDS[j+5]-sample_array_TDS[j-1];
			pH_difference  = sample_array_pH[j+5]-sample_array_pH[j-1];

			if(TDS_difference      < -0.0000000000001)	TDS_total_decreasing++;// if the TDS slope is negative increase TDS total decreasing
			else if(TDS_difference >  0.0000000000001)	TDS_total_increasing++;// if the TDS slope is positive increase TDS total increasing
			else if(TDS_difference == 0)				TDS_total_same++;

			if(pH_difference 	  < -0.009)	pH_total_decreasing++;// if the pH slope is negative increase pH total decreasing
			else if(pH_difference >  0.009)	pH_total_increasing++;// if the pH slope is positive increase pH total increasing
			else if((pH_difference >= -0.009 && pH_difference <= 0.009) || pH_difference == 0) pH_total_same++;
	}*/
	...
	/*
	float prev_run_difference_large = largest_value_pH - prev_largest_pH;
	float prev_run_difference_small = smallest_value_pH - prev_smallest_ph;
	float difference_factor_prev_run_pH = prev_run_difference_large-prev_run_difference_small;
	prev_run_difference_large = largest_value_TDS - prev_largest_TDS;
	prev_run_difference_small = smallest_value_TDS - prev_smallest_TDS;
	float difference_factor_prev_run_TDS = prev_run_difference_large-prev_run_difference_small;
	//prev_largest_TDS = largest_value_TDS;
			//prev_largest_pH = largest_value_pH;
*/
...
// if the smallest values location is after the largest value and we are decreasing
	// (pH_smallest_loc > pH_largest_loc+10 && slope_factor_ph > 0.001) ||(TDS_smallest_loc > TDS_largest_loc+10 && slope_factor_TDS > 15) |||| slope_factor_ph > 0.01 || slope_factor_ph < -0.01 || slope_factor_TDS > 5 || slope_factor_TDS < -5 || pH_total_same < 9
	//|| difference_factor_prev_run_pH > 0.029 || difference_factor_prev_run_pH < -0.029 || difference_factor_prev_run_TDS > 10 || difference_factor_prev_run_TDS < -10 ||
	if(run_again != 2 && (historic_sample_index <= 7 || TDS_range > 12.0 || pH_range > 0.06 || historic_range_pH > 0.07 || historic_range_pH < -0.07 || historic_range_TDS > 15.0 || historic_range_TDS < -15.0 || historic_average_TDS_range > 3.0|| historic_average_TDS_range < -3.0 || historic_average_pH_range > 0.02 || historic_average_pH_range < -0.02 ))
	{
 		valid = 0;
 		recheck_count++;
...
float waitForWaterToStabilize() // Returns the total time in seconds
{
	...
	total_time_seconds = eq_end_time_sec-eq_start_time_sec;
	stability_value = 0;
	recheck_count = 0;
	resetStabilityVars();
...
void calibrateSys()
{
	...
	if(initial_run == 1)				 						// if the first run
	{
		...
		for(int testCount = 0; testCount < 5; testCount++)
		{
			...
			total_nutrient_ml += nutrient_calibration_dose;
			//	total_time_seconds = waitForWaterToStabilize();		// Returns the total time in seconds
			sprintf(converted_char, "%.2f", (double)total_nutrient_ml);	// convert water temp to char array and write it to the data buffer
	...
	if(up_down_total == 3) 										// if we wrote both ph-up and ph-down files dose with nutrients
	{
		...
		total_nutrient_ml += nutrient_calibration_dose;
		if(total_nutrient_ml >= 3*nutrient_calibration_dose)ph_calibration_dose++; // if we have added 2 nutrient doses increase the pH-up/down dose since it takes more to equalize
		total_time_seconds = waitForWaterToStabilize();			// Returns the total time in seconds
		saveStateAfterNutrient(total_time_seconds);
		set_init_water_ph();
		up_down_total = 0;
...
void writeCalibrationData() // file refers to the document to be written (pH-down characteristcs=0, pH-up=1)
{
	...
	if(write_times < number_of_files)
	{
		...
		sprintf(total_nutrient_string, "%d", total_nutrient_ml);	  // convert the total nutrient volume so we can create the file name
